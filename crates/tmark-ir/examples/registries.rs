...
//! container names, fragment contracts and keystroke labels.
...
use serde_json::json;
use tmark_ir::registry::{
    ADMONITIONS, CONTAINERS, FRAGMENTS, KEY_LABELS, NODE_WORDS, PREFIXES, ROLES,
};
...
fn main() {
    ...
    let value = json!({
        ...
        "containers": CONTAINERS.iter().map(|c| c.name).collect::<Vec<_>>(),
