...
| C19 | §Structure, §Raw | Inline and block HTML other than comments have no catalogue entry; the printer rewrites them to `html raw` fences / `{raw html}` (class D) and a leading comment swallows its line. | Decide the canonical spelling (kept as typed, class C). | spec |
...
| C25 | §Lexical grammar, quoted values | A quoted attribute or fence-option value is `"[^"]*"`: `"` cannot appear inside one, yet the parser keeps a `\"` it meets and the printer writes `\"`, and the two are not inverses (each print doubles the backslashes; `texsmith/examples/paper.md` never reaches a fixed point). | Decide: `\"` becomes an escape inside quoted values (parser decodes, printer encodes), or `"` is unrepresentable and both sides stop pretending. | spec |
